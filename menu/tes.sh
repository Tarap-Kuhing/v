...
fun_botOnOff() {
  ...
  [[ ! -f /root/multi/bot.conf ]] && {
    echo -e "TARAP KUHING TUNNELKNG Bot Panel Installer
        "
    [[ ! -f /root/ResBotAuth ]] && {
    ...
    echo -ne "Bot Username, Dont use '@' [Ex: tarap_kuhing_bot] : "
    read bot_user
    [[ -z $bot_user ]] && bot_user="tarap_kuhing_bot"
    echo ""
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -q -O https://raw.githubusercontent.com/Tarap-Kuhing/v/main/menu/BotApi.sh >/etc/.maAsiss/.Shellbtsss
      }
