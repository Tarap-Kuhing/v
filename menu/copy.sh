...
function BACKUPVPS() {
mkdir -p /var/www/html/Tarap-Kuhing >/dev/null 2>&1
cp -r /etc/xray/config.json  config.json >/dev/null 2>&1
cp -r /etc/xray/ssh  ssh >/dev/null 2>&1
zip config.json vps-backup.zip >/dev/null 2>&1
zip ssh vps-backup.zip >/dev/null 2>&1
mv *.zip /var/www/html/Tarap-Kuhing >/dev/null 2>&1
...
function RESTOREVPS() {
mv /var/www/html/Tarap-Kuhing/*.zip vps-backup.zip >/dev/null 2>&1
rm -f *.zip >/dev/null 2>&1
mv *.json /etc/xray >/dev/null 2>&1
mv *.ssh /etc/xray >/dev/null 2>&1
